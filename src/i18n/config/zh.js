const zh = {
  ...
  brushMethods: {
    headerTitle: "刷牙步骤",
    subtip:
      "将牙刷头按正确方向插入输入轴，刷头与主体之间需保留一些缝隙，以免影响震动效果。",
    ...
    tip7: "1、了解我们的口腔",
    subtip7:
      "汇优尚品依据科学的巴氏刷牙法，将口腔分为4区16个牙面，帮您清洁每个牙面，守护牙齿健康。",
    tip8: "2、智能牙刷使用流程",
    subtip8: "①牙刷连接成功后，启动牙刷即可进入实时引导界面。",
    subtip9:
      "② 将牙刷放在起始牙区开始清洁牙区的每个牙面。磨牙区分3个牙面，切牙区分2个牙面。每个牙面都要均匀清洁，才能清洁牙齿获得更高得分哦。",
    subtip10: "③ 牙面清洁时，请在当前牙区刷牙。",
    subtip11:
      "④ 根据清洁牙区方向及提示切换一牙区，暂停牙刷后可结束刷牙，结算本次刷牙分数。",
    subtip12:
      "PS：每次刷牙要保持2分钟以上的时间，低于30秒的刷牙结果不会被计算哦。",
  },
  //常见问题
  CommonProbl: {
    title2: "常见问题",
    tip: "1、蓝牙已开启，搜索不到蓝牙",
    subtip:
      "同一账号在搜索牙刷时，已被此账号绑定的牙刷将不能再次在搜索列表中显示。",
    tip2: "2、数据缺失",
    subtip2: "刷牙时间低于30秒的刷牙数据为无效刷牙数据，无法保存及同步。",
    tip3: "3、历史数据存储与保留",
    subtip3:
      "为保护用户隐私，已被绑定的牙刷在被另一账号绑定时，会清除机身离线刷牙数据;同一账号在删除已绑定的牙刷时，将会清除所有云端历史数据刷牙数。",
    tip4: "4、为什么APP没有数据",
    subtip4: "1、APP未连接蓝牙：请确保蓝牙时打开状态。",
    subtipMore: "2、请确保手机蓝牙没有连接其他蓝牙设备。",
    tip5: "5、实时引导注意事项",
    subtip5:
      "实时引导时不建议将智慧生活APP置于后台运行，此行为将会导致刷牙时间与实际不符，实时引导期如需离开可将牙刷暂停后结束刷牙。",
    tip6: "6、特别提醒",
    subtip6:
      "请勿在修改手机系统内日期/时间后连接牙刷使用，此做法会造成刷牙记录时间混乱，或离线数据无法同步。如遇此问题请校正系统时间后通过删除设备重新添加解决。",
  },
  // 刷牙剩余使用时间 重置
  RemainTime: {
    header: "刷头剩余使用时间",
    ...
    proposal:
      "每次刷牙时间超过30秒即被记录为一次有效刷牙次数，每只刷头可用120次（每天两次可用60天），超过使用天数及时更换刷头",
    rest: "刷头剩余",
